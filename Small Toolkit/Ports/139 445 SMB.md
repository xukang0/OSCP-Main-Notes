...
const command = `smbmap -H ${ip} -u [user] -p '[pw]'`;
...
const command = `enum4linux ${ip}`;

dv.paragraph("```bash\n" + command + "\n```");
```
#### Credential SMB Enumeration
```dataviewjs
const page = dv.page("Synced OSCP Notes/Top/Active Machine");const ip = page?.IP ?? "NO IP FOUND";

const command = `enum4linux-ng ${ip} -u "[Username]" -p "[Password]"`;
...
const command = `smbclient -U [username] -L //${ip}/`;
...
const command = `smbclient //${ip}/[sharename] -c 'recurse;ls'`;

dv.paragraph("```bash\n" + command + "\n```");
```
#### TEST IF CAN PLACE FILE INTO THIS SHARE (Write access)

```
put test.txt
```

IF YES, [[NTLM Auth]], or get a reverse shell by putting nc64.exe and shell.php webshell
...
- ADMIN is a hidden admin share and we need admin rights
- C$ is the entire C drive and admin only too
- IPC is used for named pipes/authentication
- NETLOGON is used by domain controllers for login scripts
- SYSVOL is Domain controller shared policies and scripts
- Password Audit is interesting
...
const command = `smbclient -U 'V.Ventz' '//${ip}/Password Audit'`;
...
const command = `smbclient -U '[user]%[pw]' //${ip}/[sharename] -c 'prompt OFF; recurse ON; mget *'`;
