...
const command = `sudo nmap -n -sS -p- -T4 -Pn -A -v ${ip}`;
...
dv.paragraph("```bash\n" + command + "\n```");
```
Slightly better scan for Boxes 
```dataviewjs
const page = dv.page("Templater/IP");const ip = page?.IP ?? "NO IP FOUND";

const command = `sudo nmap -sV --open -oA [filename] ${ip}`;

dv.paragraph("```bash\n" + command + "\n```");
```
Scripts scan for Identified Ports
```dataviewjs
const page = dv.page("Templater/IP");const ip = page?.IP ?? "NO IP FOUND";

const command = `sudo nmap -sC -p [portno] -oA [filename] ${ip}`;

dv.paragraph("```bash\n" + command + "\n```");
```
Web application directory script
```dataviewjs
const page = dv.page("Templater/IP");const ip = page?.IP ?? "NO IP FOUND";

const command = `sudo nmap -sV --script=http-enum -oA [filename] ${ip}`;
